    ...
    public function index()
    {
        $userStats = User::leftJoin('golf_rounds', 'users.id', '=', 'golf_rounds.user_id')
            ->select(
                ...
                DB::raw('ROUND(COALESCE(SUM(golf_rounds.birdies), 0)::numeric / NULLIF(SUM(CASE WHEN golf_rounds.holes_played = 9 THEN 0.5 ELSE 1 END), 0), 2) as avg_birdies_per_round'),
                DB::raw('ROUND(COALESCE(SUM(golf_rounds.eagles), 0)::numeric / NULLIF(SUM(CASE WHEN golf_rounds.holes_played = 9 THEN 0.5 ELSE 1 END), 0), 2) as avg_eagles_per_round'),
                DB::raw('ROUND(COALESCE(SUM(golf_rounds.putts), 0)::numeric / NULLIF(SUM(CASE WHEN golf_rounds.holes_played = 9 THEN 0.5 ELSE 1 END), 0), 2) as avg_putts_per_round')
            )
        ...
        // Top 10 by birdies
        $topBirdies = User::leftJoin('golf_rounds', 'users.id', '=', 'golf_rounds.user_id')
            ->select(
                'users.name',
                DB::raw('COALESCE(SUM(golf_rounds.birdies), 0) as total_birdies')
            )
        ...
        // Top 10 by rounds
        $topRounds = User::leftJoin('golf_rounds', 'users.id', '=', 'golf_rounds.user_id')
            ->select(
                'users.name',
                DB::raw('SUM(CASE WHEN golf_rounds.holes_played = 9 THEN 0.5 ELSE 1 END) as total_rounds')
            )
        ...
        return view('public-stats', compact('userStats', 'aggregateStats', 'topBirdies', 'topEagles', 'topRounds'));
    }
}
